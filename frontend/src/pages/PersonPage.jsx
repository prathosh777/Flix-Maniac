...
{movies.length > 0 && (
  ...
  >
    <h3 className="text-2xl ml-0 md:ml-36 font-bold mb-4">Movies</h3>
    <div className="relative mx-auto w-[100%] md:w-[80%]"> 
      <div
        ref={sliderRef}
        className="flex overflow-x-scroll gap-4 pb-4 scrollbar-hide"
      >
        {movies.map(
          (movie) =>
            movie.poster_path && (
      ...
      <ChevronLeft
        className="absolute top-[50%] -translate-y-1/2 left-0 w-10 h-10 bg-white text-gray-800 p-2 rounded-full cursor-pointer hover:bg-gray-200 z-10"
        onClick={scrollLeft}
      />

      <ChevronRight
        className="absolute top-[50%] -translate-y-1/2 right-0 w-10 h-10 bg-white text-gray-800 p-2 rounded-full cursor-pointer hover:bg-gray-200 z-10"
        onClick={scrollRight}
      />
    </div>
  </motion.div>
